function err = adapt_asc_sel(ref_sensor, rep_sensor, new_sensor, ground_truth)

% pre-select reference sensors
...
if size(corMat,1) == 1
    ...
    corVal = sum( abs(corMat) );
end

[tmp, sel_idx] = sort(corVal,'descend');


ref_sensor = ref_sensor(sel_idx(1:20),:);
