function detect_changes_local(input_data, col_id, row_id)
...
%% find local training data
BLOCK = 1000;
N_BLOCK = floor(size(test_x,1) / BLOCK);
train_idx = [];
for i = 1:N_BLOCK+1
    i
    if i <= N_BLOCK
        ...
        block_idx = BLOCK*(i-1)+1:size(test_x,1);
    end
    dist = L2_distance(test_x(block_idx,:)', train_x');
    [dist, sub_idx] = sort(dist,2);
    
    pp = sub_idx(:,1:3);
    train_idx = [train_idx; pp(:)];
end
