function classify_metricL(source, target, low_dim, lambda, type)
...
if type == 1
    fname = ['result1/', source, '_', target, '_dim_', num2str(low_dim), '_reg_', num2str(lambda), '_type_', num2str(type), '_L.mat'];
else
    fname = ['result2/', source, '_', target, '_dim_', num2str(low_dim), '_reg_', num2str(lambda), '_type_', num2str(type), '_L.mat'];
end
...
if type == 1
    fname = ['result_accu1/', source, '_', target, '_dim_', num2str(low_dim), '_reg_', num2str(lambda), '_type_', num2str(type), '_L_accu.mat'];
else
    fname = ['result_accu2/', source, '_', target, '_dim_', num2str(low_dim), '_reg_', num2str(lambda), '_type_', num2str(type), '_L_accu.mat'];
end
